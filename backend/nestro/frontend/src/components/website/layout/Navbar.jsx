      ...
      <nav className="hidden md:flex items-center gap-1">
        {navLinks.map((link) => (
          <Link
            key={link.href}
            href={link.href}
            className={`text-[11px] tracking-[0.06em] rounded-md px-3 py-1.5 transition-colors ${
              pathname === link.href
        ...
        <div className="absolute top-[58px] left-0 right-0 bg-[#FAFAF9] border-b border-[#E8E0D5] flex flex-col px-4 py-3 gap-1 md:hidden z-[299]">
          {navLinks.map((link) => (
            <Link
              ...
              className={`text-[12px] tracking-[0.06em] rounded-md px-3 py-2 w-full transition-colors ${
                pathname === link.href
                  ? 'bg-[#F0EBE3] text-[#8B5E3C]'
                  : 'text-[#6B7280] hover:bg-[#F0EBE3] hover:text-[#8B5E3C]'
              }`}
